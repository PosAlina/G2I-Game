...
float UG2IVerticalMovementComponent::FindTargetLocation() const
{
	if (!ensure(Owner)) {
	...
	const FVector Origin = TargetMeshComponent->Bounds.Origin;
	const FVector BoxExtent = TargetMeshComponent->Bounds.BoxExtent;
	...
	constexpr ECollisionChannel TraceChannel = ECC_WorldStatic;
	...
	for (const FVector& StartPoint : StartPoints) {
		...
		if (World->LineTraceSingleByChannel(Hit, StartPoint, EndPoint, TraceChannel, Params)) {
			const float DistToHit = StartPoint.Z - Hit.Location.Z;
...
void UG2IVerticalMovementComponent::MoveDown()
{
	...
	const float DeltaZ = FindTargetLocation();
...
void UG2IVerticalMovementComponent::BeginPlay()
{
	...
	// Not necessary, only for grabber
	LauncherComp = Owner->FindComponentByClass<UG2ILauncherComponent>();
}
